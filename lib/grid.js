...
(function() {
  'use strict';
  var CELLS, CND, LETTERS, alert, assign, badge, contains, debug, echo, help, info, jr, log, new_xregex, rpr, urge, warn, whisper;
  ...
  // ITERATORS
  //-----------------------------------------------------------------------------------------------------------
  this.walk_cells_from_keys = function*(grid, keys) {
    var cell, i, key, len, ref, seen_cellkeys;
    if (CND.isa_text(keys)) {
      yield* this.walk_cells_from_keys(grid, keys.split(/\s*,\s*/g));
      return;
    }
